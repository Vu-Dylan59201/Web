...
public class DAO {
	...
	public DAO() {
		try {
			...
			con = DriverManager
					.getConnection("jdbc:sqlserver://localhost:1433;databasename=TBBEP;username=sa;password=123456");
		} catch (Exception e) {
	...
	public boolean insertSanPham(SanPham sp) {
		try {
			PreparedStatement ps = con.prepareStatement(
					"INSERT INTO SanPham (TenSP,Gia,GiaKM, IDLoai, TieuDe, MoTa, SL, URLHinh) VALUES (?,?,?,?,?,?,?,?)");

			ps.setString(1, sp.getTenSP());
			ps.setInt(2, sp.getGia());
			ps.setInt(3, sp.getGiaKM());
			ps.setInt(4, sp.getIDLoai());
			ps.setString(5, sp.getTieuDe());
	...
	public boolean updateSanPham(SanPham sp, int IDSP) {
		try {
			PreparedStatement ps = con.prepareStatement(
					"UPDATE SanPham SET TenSP = ?,Gia = ?,GiaKM = ?,IDLoai = ?,TieuDe =?,MoTa = ?,SL = ?,URLHinh = ?,SoLuotXem = ? WHERE IDSP = ?");
